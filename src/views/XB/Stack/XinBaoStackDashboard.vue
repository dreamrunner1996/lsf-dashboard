<template>
  <div class="xin-bao-stack-dashboard-content">
...
export default {
  ...
  methods: {
    ...
    GetData: async function () {
      ...
      // await GetReadData('http://localhost:1234/xb/test').then(res => {
        // 处理获取得到的参数
    ...
    DialogDataData (dataRes) {
            ...
            if (inpOutRes.param.startTime.timeStamp === dataRes.startTime.timeStamp && inpOutRes.param.endTime.timeStamp === dataRes.endTime.timeStamp) {
              ...
              if (dataRes.productList && dataRes.productList instanceof Array && dataRes.productList.length) {
                ...
                dataRes.productList.forEach(productRes => {
                  if (productRes.state === 'Input') inputNum += productRes.value
      ...
      if (dataData.dataList.length) {
        ...
        dataData.dataList.forEach(dataRes => {
          if (dataRes.productList && dataRes.productList instanceof Array && dataRes.productList.length) {
            dataRes.productList.forEach(productRes => {
              if (productRes.state === 'Input') allInput += productRes.value
              if (productRes.state === 'Ok') allOutput += productRes.value
            })
          }
        })
    ...
    DialogProtocolNamesData (protocolData) {
      ...
      if (protocolData && protocolData instanceof Object && Object.keys(protocolData).length) {
        ...
        protocolNamesArr.forEach(protocolNamesArrRes => {
          if (protocolNamesArrRes.split('StackPalletStatus').length > 1) this.$set(this.statusList[2], 'value', protocolData[protocolNamesArrRes])
          if (protocolNamesArrRes.split('StackPalletsReserve').length > 1) this.$set(this.statusList[3], 'value', protocolData[protocolNamesArrRes])
        })
      }
    },
    // 创建 投入-产出 多折线图
